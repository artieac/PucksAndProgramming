...
namespace AlwaysMoveForward.Core.Common.Encryption
{
    ...
    public class RSAEncryptionHelper
    {
        ...
        public static RSA FromXmlString(string xmlString)
        {
            RSA retVal = RSA.Create();
            ...
            // Note that we use the public key to encrypt
            retVal.ImportParameters(rsaParameters);
            return retVal;
        }
        ...
        public byte[] Encrypt(byte[] plainData, bool useOAEP, X509Certificate2 certificate)
        {
            ...
            using (RSA provider = RSAEncryptionHelper.FromXmlString(this.GetPublicKey(certificate)))
            { 
                return provider.Encrypt(plainData, null);// useOAEP);
            }
        }
        ...
        public byte[] Decrypt(byte[] encryptedData, bool useOAEP, X509Certificate2 certificate)
        {
            ...
            using (RSA provider = RSAEncryptionHelper.FromXmlString(this.GetXmlKeyPair(certificate)))
            {
                return provider.Decrypt(encryptedData, null);// useOAEP);
            }
        }
        ...
        public string GetPublicKey(X509Certificate2 certificate)
        {
            ...
            return certificate.GetRSAPublicKey().ToString();
        }
        ...
        public string GetXmlKeyPair(X509Certificate2 certificate)
        {
            ...
            {
                return certificate.GetRSAPrivateKey().ToString();
            }
        }
    }
}
